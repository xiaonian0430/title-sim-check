...
if "SYNONYMS_DEBUG" in ENVIRON and ENVIRON["SYNONYMS_DEBUG"].lower() == "true":
    _debug = True
        ...
        if y_ not in _stopwords:
            ...
            try:
                ...
                if ignore:
                    ...
                    _logging_debug("not exist in w2v dao: %s" % y_)
...
def compare(s1, s2, segment=True, ignore=False, stopwords=False):
    """
    compare similarity
    s1 : sentence1
    s2 : sentence2
...
def describe():
    ...
    print("Vocab size in vector dao: %d" % vocab_size)
